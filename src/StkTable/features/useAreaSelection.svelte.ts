...
import { getClosestColKey, getClosestTd, getClosestTr, getClosestTrIndex } from '../utils/index';
...
/**
 * useAreaSelection 依赖注入。
 * 由于 Svelte 5 的响应式基于「读取时追踪」，这里统一用 getter 暴露主组件的 $state/$derived，
 * feature 内部在 $derived/$effect 中调用 getter 即可建立响应式依赖。
 */
export type UseAreaSelectionDeps<DT extends Record<string, any>> = {
    /** 读取 areaSelection prop（boolean | 配置对象） */
    getAreaSelectionProp: () => boolean | AreaSelectionConfig | undefined;
    /** 读取 scrollRowByRow prop */
    getScrollRowByRow: () => boolean | 'scrollbar' | undefined;
    /** 触发 area-selection-change 事件 */
    emitChange: (ranges: AreaSelectionRange[]) => void;
    /** 读取表格容器元素 */
    getContainer: () => HTMLElement | undefined;
    /** 读取处理后的数据源（已排序/展平/筛选） */
    getDataSource: () => DT[];
    /** 读取最后一行表头列（叶子列） */
    getTableHeaderLast: () => StkTableColumn<DT>[];
    /** 列 key 生成器 */
    colKeyGen: (col: StkTableColumn<DT>) => string;
    /** 单元格 key 生成器 */
    cellKeyGen: (row: DT | null | undefined, col: StkTableColumn<DT>) => string;
    /** 滚动到指定位置 */
    scrollTo: (top: number | null, left: number | null) => void;
    /** 读取虚拟滚动 Y 状态 */
    getVirtualScroll: () => VirtualScrollStore;
    /** 读取虚拟滚动 X 状态 */
    getVirtualScrollX: () => VirtualScrollXStore;
    /** 行对象 → 绝对索引 */
 ...
 */
export function useAreaSelection<DT extends Record<string, any>>(deps: UseAreaSelectionDeps<DT>) {
    /**
    ...
    // hover 时框架的 class patch 会用整体覆盖 className，从而抹掉命令式添加的选区 class，
    // 导致选区高亮/边框在鼠标经过合并单元格时消失。
    // data-* 属性不在响应式 props 中，框架不会清除它们，可稳定保留选区样式。
    ...
    const config = $derived.by<AreaSelectionConfig>(() => {
        const prop = deps.getAreaSelectionProp();
        if (typeof prop === 'boolean') {
            const b = prop;
            return { enabled: b, keyboard: b, ctrl: b, shift: b, highlight: { cell: b, row: false } };
        }
        const { highlight: userHighlight, ...restConfig } = prop || {};
        return {
            enabled: true,
            ctrl: true,
            shift: true,
    ...
    /** 是否启用键盘控制选区移动 */
    const keyboardEnabled = $derived(config.keyboard);

    /** 是否启用 Ctrl 多选 */
    const ctrlEnabled = $derived(config.ctrl);

    /** 是否启用 Shift 扩选 */
    const shiftEnabled = $derived(config.shift);

    /** 是否启用单元格高亮 */
    const highlightCellEnabled = $derived(config.highlight?.cell);

    /** 是否启用行高亮 */
    const highlightRowEnabled = $derived(config.highlight?.row);

    /** colKey → absolute index 映射 */
    const colKeyToIndexMap = $derived.by(() => {
        const headers = deps.getTableHeaderLast();
        const map = new Map<UniqKey, number>();
        for (let i = 0; i < headers.length; i++) {
            map.set(deps.colKeyGen(headers[i]), i);
        }
        return map;
    });
     ...
     */
    const getFixedColWidths = $derived.by(() => {
        const cols = deps.getTableHeaderLast();
        ...
        for (let i = 0; i < cols.length; i++) {
            ...
            if ((cols[i] as any)?.fixed === 'left') {
                leftSum += getCalculatedColWidth(cols[i] as any);
            }
        }
        ...
        for (let i = cols.length - 1; i >= 0; i--) {
            ...
            if ((cols[i] as any)?.fixed === 'right') {
                rightSum += getCalculatedColWidth(cols[i] as any);
            }
        }
        ...
        for (const range of ranges) {
            ...
            for (let r = rStart; r <= rEnd; r++) {
                ...
                for (let c = cStart; c <= cEnd; c++) {
                    const col = cols[c];
                    if (col) keys.add(deps.cellKeyGen(row, col));
                }
            }
        }
    ...
    /**
     * 直接操作 DOM 更新选区样式（不触发框架 re-render）
     * 用于替代在渲染中读取响应式选区数据，避免拖选时频繁触发整个表格重渲染
     */
    function updateSelectionDOM() {
        const container = deps.getContainer();
        if (!container) return;

        const cellHighlight = untrack(() => highlightCellEnabled);
        const rowHighlight = untrack(() => highlightRowEnabled);
        ...
        if (cellHighlight) {
            ...
            for (let t = 0; t < trs.length; t++) {
                ...
                for (let d = 0; d < tds.length; d++) {
                    ...
                    const ck = deps.cellKeyGen(row, cols[colIndex]);
    ...
    /**
     * 监听选区变化与虚拟滚动可见范围变化，在 DOM 更新后直接操作选区样式。
     * 等效 Vue 版的 watch(..., { flush: 'post' }) + nextTick(updateSelectionDOM)。
     */
    $effect(() => {
        // 读取依赖建立追踪：选区变化 或 虚拟滚动可见范围变化 都触发
        const ranges = selectionRanges;
        const vs = deps.getVirtualScroll();
        const vsx = deps.getVirtualScrollX();
        void ranges.length;
        void (ranges.length > 0 ? JSON.stringify(ranges.map(r => r.index)) : '');
        void vsx.scrollLeft;
        void vs.startIndex;
        void vs.endIndex;
        void vsx.startIndex;
        void vsx.endIndex;
        void deps.getDataSource().length;
        void deps.getTableHeaderLast().length;

        // tick 后更新 DOM（此时框架已完成 patch）。
        // then 回调在微任务中执行，无活动 effect 上下文，内部读取不会被追踪。
        tick().then(updateSelectionDOM);
    });

    /**
     * 容器就绪后挂载 keydown 监听；容器变化或组件销毁时自动清理。
     * 等效 Vue 版的 onMounted(addListener)。
     */
    $effect(() => {
        const container = deps.getContainer();
        if (!container) return;
    ...
    /** 组件销毁时移除 document 级监听并停止自动滚动（等效 onBeforeUnmount(removeListener)） */
    onDestroy(() => {
        removeListener();
    });
     ...
     */
    $effect(() => {
        const rowCount = deps.getDataSource().length;
        const colCount = deps.getTableHeaderLast().length;
        untrack(() => {
    ...
    function getColPosition(colIndex: number): [number, number] {
        ...
        const cols = deps.getTableHeaderLast();
        for (let i = 0; i < cols.length; i++) {
            const colWidth = getCalculatedColWidth(cols[i] as any);
    ...
    function emitSelectionChange() {
        deps.emitChange(selectionRanges);
    }
    ...
    function onKeydown(e: KeyboardEvent) {
        ...
        if (!selectionRanges.length) {
            ...
            selectionRanges = [makeRange(0, 0, 0, 0)];
            emitSelectionChange();
            scrollToCell(0, 0);
            return;
        }
        ...
        if (e.shiftKey && isArrowKey && shiftEnabled) {
            ...
            selectionRanges = [makeRange(newRow, newCol, newRow, newCol)];
    ...
    function scrollToCell(rowIndex: number, colIndex: number) {
        ...
        const row = deps.getDataSource()[rowIndex];
        const col = deps.getTableHeaderLast()[colIndex];
        if (!row || !col) return;
        ...
        const isScrollRowByRow = deps.getScrollRowByRow();
        ...
        if (newScrollTop !== null || newScrollLeft !== null) {
            deps.scrollTo(newScrollTop, newScrollLeft);
        }
    }
    ...
    function setAreaSelection(ranges?: AreaSelectionSetterRange<DT>, option: AreaSelectionSetterOption = {}): AreaSelectionRange[] {
        ...
        if (ranges) {
            ...
            beginRow = typeof begin.row === 'number' ? begin.row : deps.getRowIndex(begin.row);
            endRow = typeof end.row === 'number' ? end.row : deps.getRowIndex(end.row);

            const beginColInput = typeof begin.col === 'number' ? begin.col : begin.col ? deps.getColumnIndex(begin.col) : void 0;
            const endColInput = typeof end.col === 'number' ? end.col : end.col ? deps.getColumnIndex(end.col) : void 0;
        ...
        selectionRanges = [makeRange(beginRow, beginCol, endRow, endCol)];
    ...
    return {
        // 用 getter 暴露，保持响应式（调用方通过 feature.config / feature.isSelecting 访问）
        get config() {
            return config;
        },
        get isSelecting() {
