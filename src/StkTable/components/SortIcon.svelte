<script lang="ts">
    let { class: className, ...rest }: { class?: string } & Record<string, any> = $props();
</script>

<svg xmlns="http://www.w3.org/2000/svg" width="16px" height="16px" viewBox="0 0 16 16" class={className} {...rest}>
    <polygon class="arrow-up" fill="#757699" points="8 2 4.8 6 11.2 6"></polygon>
    <polygon class="arrow-down" transform="translate(8, 12) rotate(-180) translate(-8, -12) " points="8 10 4.8 14 11.2 14"></polygon>
</svg>
