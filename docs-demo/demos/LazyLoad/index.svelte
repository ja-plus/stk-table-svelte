<script lang="ts">
    ...
    function initPlaceholderData() {
        ...
        tableData = placeholders;
    }

    initPlaceholderData();

    const loadedCount = $derived(tableData.filter(item => !item.__placeholder).length);
    ...
    async function loadDataPage(page: number) {
        ...
        try {
            ...
            response.forEach((item, index) => {
                if (startIndex + index < tableData.length) {
                    tableData[startIndex + index] = item;
                }
            });
...
<div>
    <header>
        <button class="btn" onclick={gcData}>GC</button>
    </header>
    <StkTable
        rowKey="id"
        style="height: 400px"
    ...
    <footer style="margin-top: 16px">
        ...
        <span style="margin-left: 16px">startIndex: {startIndex} endIndex: {endIndex}</span>
    </footer>
</div>
