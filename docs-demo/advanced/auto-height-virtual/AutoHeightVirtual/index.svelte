<script lang="ts">
    ...
    // 环境未安装 mockjs，用本地随机文本生成器等价替代 mockjs.Random 的 csentence/sentence/cparagraph/paragraph/datetime
    ...
    /** 等价 mockjs.Random.cparagraph(min, max) */
    ...
    /** 等价 mockjs.Random.paragraph(min, max) */
    ...
    /** 等价 mockjs.Random.datetime(format) */
    ...
    const data = new Array(50).fill(0).map((_, i) => ({
        id: i,
        title: isZH ? csentence(1, 5) : sentence(1, 5),
        content: isZH ? cparagraph(2, 15) : paragraph(1, 5),
        date: isZH ? datetime('yyyy-MM-dd HH:mm') : datetime('MM/dd/yyyy HH:mm'),
    }));
</script>
