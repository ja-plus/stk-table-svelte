...
<demo svelte="advanced/custom-cells/ChangeCell/index.svelte" github="https://github.com/ja-plus/stk-table-svelte/tree/master/docs-demo/advanced/custom-cells/ChangeCell/index.svelte"></demo>

```ts
const { ChangeCell } = createChangeCell({ decimals: 2, showSign: true, arrow: true });

const columns = [
    { title: 'Change', dataIndex: 'change', align: 'right', customCell: ChangeCell },
];
```
