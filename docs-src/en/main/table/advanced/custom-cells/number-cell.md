...
<demo svelte="advanced/custom-cells/NumberCell/index.svelte" github="https://github.com/ja-plus/stk-table-svelte/tree/master/docs-demo/advanced/custom-cells/NumberCell/index.svelte"></demo>

```ts
const { NumberCell } = createNumberCell({ decimals: 2 });

const columns = [
    { title: 'Price', dataIndex: 'price', align: 'right', customCell: NumberCell },
];
```
...
```ts
import { formatNumber } from 'stk-table-svelte';
