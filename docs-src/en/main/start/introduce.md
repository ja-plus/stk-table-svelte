# Introduce
`stk-table-svelte` (stk: sticky) is a **high-performance** virtual list based on `sticky`, `Svelte`, and `DOM`. It is suitable for dynamic data display scenarios.


## Why stk-table-svelte was Created
...
3. **DOM-based.** Being DOM-based facilitates seamless integration with Svelte component libraries. Moreover, I have great confidence in DOM performance. After considering various costs, I abandoned the canvas implementation.
...
5. **Declarative Custom Slots.** Borrowed from `ant-design-vue` table's custom cell approach, which is much more elegant than using custom cells via `snippet` in the template.
