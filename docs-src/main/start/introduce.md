# 介绍
`stk-table-svelte`（stk: sticky） 是一个基于 `sticky` 、`Svelte`、`DOM` 的**高性能**虚拟列表。适用于动态数据展示场景。


## 为什么要做stk-table-svelte
...
3. **基于DOM。** 基于 DOM 有利于 Svelte 组件库的无缝接入。而且，我十分相信dom的性能。综合各种成本考虑，舍弃了canvas的实现。
...
5. **声明式自定义插槽。** 借鉴了 `ant-design-vue` 表格的自定义单元格的方式，相比在模板中通过 `snippet` 使用自定义单元格要优雅许多。
