...
点击表头后，会触发 `onsortchange` 回调，您可以在回调中发起 ajax 请求，然后重新对 `props.dataSource` 赋值，完成排序。
...
type SortConfig<T extends Record<string, any>> = {
    ...
    defaultSort?: {
        ...
        /** 是否禁止触发 onsortchange 回调，默认 false */
        silent?: boolean;
    };
    /** 空值始终排在列表末尾 */
...
| defaultSort.silent | `boolean` | `false` | 是否禁止触发 `onsortchange` 回调。 |
